...
from hat_llm.slot_packet_io import load_slot_packet_json, slot_packet_to_jax_arrays
...
def _build_slot_inputs(
    ...
    if slot_packet_path:
        packet = load_slot_packet_json(slot_packet_path)
        return tokenizer, slot_packet_to_jax_arrays(packet, tokenizer, slot_max_length, slot_max_count)
...
def run_inference(model_dir: str, repo_root: str, specialist_id: str, runtime_mode: str, prompt: str, slot_packet_path: str = "") -> str:
    cfg = _load_config(model_dir)
    effective_slot_packet = slot_packet_path or str(cfg.get("slot_packet_path", ""))
    tokenizer, slot_inputs = _build_slot_inputs(
        ...
        slot_packet_path=effective_slot_packet,
    )
...
def main() -> None:
    ...
    print(run_inference(args.model_dir, args.repo_root, args.specialist_id, args.runtime_mode, args.prompt, args.slot_packet))
